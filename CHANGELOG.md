...
## v0.22.0 — 2026-05-15 — Remove KillerApplication signature spoofer (Android 15 arm64 crash root cause)

### Cause identified by 3 parallel Opus audit agents

User reports app crashes immediately on OnePlus 12 / Android 15 / arm64-v8a;
boots fine on x86_64 redroid emulators (Android 13 AND Android 15). Server
log shows boot ping uploaded from OnePlus device but no crash trace.

Three parallel Opus subagents reading actual smali, manifest, native libs,
DEX files, and static-analysis-tool output independently converged on the
same root cause:

**`bin/mt/signature/KillerApplication` + `org/lsposed/hiddenapibypass/*`**
— the cracker's signature spoofer chain. It was set as the super-class of
`com.mplus.lib.ui.main.App`. On class init it reflects into ART internals
(`artMethod`, `iFields`, `sFields`, etc.) and reflectively writes
`PackageInfo.CREATOR` to substitute the chompSMS signing cert at runtime.

On Android 15 with OEM hardening (OxygenOS/ColorOS on OnePlus 12), ART
internal field layout shifted and/or hidden-API enforcement now blocks
the reflection. `HiddenApiBypass.<clinit>` throws
`ExceptionInInitializerError` → poisons `KillerApplication` → poisons
`App` → process death via system path that runs BEFORE our default
uncaught handler can fire.

Redroid emulators run permissive AOSP without OEM hardening, so the same
spoofer succeeds there.

### Fix
- `textra_base/smali_classes2/com/mplus/lib/ui/main/App.smali` line 2:
  `.super Lbin/mt/signature/KillerApplication;` → `.super Landroid/app/Application;`
- Deleted `textra_base/smali_classes3/bin/mt/signature/` (KillerApplication
  + KillerApplication$1).
- Deleted `textra_base/smali_classes3/org/lsposed/hiddenapibypass/` (10
  classes — Helper$AccessibleObject, $Class, $Executable, $HandleInfo,
  $InvokeStub, $MethodHandle, $MethodHandleImpl, $NeverCall + HiddenApiBypass).
- Grep verified zero remaining references to either class from the rest
  of the app.

### Why we can safely remove the spoofer
- PairIP (the original anti-tamper) is already absent in this APK — the
  cracker removed it. The spoofer was there to fake the original cert for
  any leftover internal signature check, but with PairIP gone there is
  nothing on this code path to deceive.
- We sign with our own keystore (textrcs.keystore). The app is what it
  claims to be. No deception needed.
- AUDIT_REPORT.md §5b already recommended this exact removal as
  pre-ship cleanup.

### Verified on both emulators
- redroid13-pairip (Android 13 x86_64): PID 3894, no FATAL, boot ping
  uploaded `17:35:02.188Z`.
- redroid15-gallery (Android 15 x86_64): PID 17186, no FATAL, boot
  pings still uploading.
- Build: `textra2_v0.22.0.apk` (73 MB).

### To verify the fix on OnePlus 12
1. Install `textra2_v0.22.0.apk`.
2. Launch the app — if our diagnosis is correct, it should reach
   PairingActivity successfully and a boot ping (or full app UI) will
   appear on server.
3. If it still crashes immediately and no crash log uploads, the cause is
   something else and we'll need to add a native signal handler.

### Cited evidence
- Agent 1 (Kotlin code audit): Finding 7 named hidden-API blocklist on
  Android 15 breaking KillerApplication reflection.
- Agent 2 (Cracker scaffolding audit): Finding 1 traced exact smali path
  through `killPM` → `Field.set(PackageInfo.CREATOR)` → catch +
  re-throw → ExceptionInInitializerError.
- Agent 3 (Static analysis tools): Finding F1 (HiddenApiBypass.<clinit>
  failure via `artMethod`/`iFields` lookups). Confirmed native libs are
  byte-identical to pristine, DEX is intact, 16KB-aligned, signed correctly.
