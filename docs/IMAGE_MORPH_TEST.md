...
1. Install the trial: `adb install -r textra2_v1.02.0.apk`
...
4. **Pinch** to zoom in, **drag** to pan while zoomed.
5. **Single-tap** the fullscreen image → it should morph back down into the
   thumbnail.
...
## Known v1-trial limits (by design, not bugs)
- **No swipe between photos yet** — v1 shows only the tapped image. Swipe is the
  planned v2 (needs a bundled/custom pager — ViewPager2 isn't in the APK).
- **Back button**: tap-to-close is the intended dismiss. Hardware/gesture Back
  may instead close the conversation while the overlay is up (the overlay can't
